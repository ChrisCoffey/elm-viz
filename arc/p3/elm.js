...
Elm.CounterList.make = function (_elm) {
   ...
   model) {
      return function () {
         ...
         _L.fromArray([]),
         A2($Basics._op["++"],
         _L.fromArray([remove,add]),
         counters));
      }();
   });
   var actions = $Signal.mailbox(Add);
   var init = function (ls) {
      return function () {
         var toModel = function (_v11) {
            return function () {
               switch (_v11.ctor)
               {case "_Tuple2":
                  return {ctor: "_Tuple2"
                         ,_0: _v11._0
                         ,_1: $Counter.init(_v11._1)};}
               _U.badCase($moduleName,
               "on line 19, column 6 to 34");
            }();
         };
         return {_: {}
                ,counters: A2($List.map,
                toModel,
                ls)
                ,nextId: A3($List.foldr,
                F2(function (_v15,acc) {
                   return function () {
                      switch (_v15.ctor)
                      {case "_Tuple2":
   ...
   update,
   init(_L.fromArray([{ctor: "_Tuple2"
                      ,_0: 98
                      ,_1: 1000}])),
   actions.signal);
